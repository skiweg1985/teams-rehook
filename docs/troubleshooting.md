...
| Bot Access group search or member view returns `Authorization_RequestDenied` | Missing Graph group or group-member read permission, or missing admin consent | Check Entra app registration API permissions and Settings readiness | Add Microsoft Graph application permissions `Group.Read.All` and `GroupMember.Read.All`, then grant tenant admin consent. Keep `User.Read.All` too for sender membership checks, or use the broader `Directory.Read.All`. |
